...
function requestDate(){return mode==="calendar"&&selectedCalendarDate?selectedCalendarDate:dateKey(selectedDay);}
function prettyDateKey(value){const d=new Date(String(value)+"T12:00:00");if(Number.isNaN(d.getTime()))return String(value||"");return new Intl.DateTimeFormat(sportLocale(),{weekday:"long",day:"2-digit",month:"2-digit",year:"numeric"}).format(d);}
...
function detailIcon(type){return type==="goal"?"⚽":type==="yellow"?"🟨":type==="red"?"🟥":"•";}
function detailHtml(key){
 if(!expandedDetails.has(key))return "";
 if(detailLoading.has(key))return '<div class="sport-match-details"><div class="muted">'+esc(st("refreshing"))+'</div></div>';
 const rows=matchDetails.get(key)||[];
 if(!rows.length)return '<div class="sport-match-details"><div class="muted">'+esc(st("noDetails"))+'</div></div>';
 return '<div class="sport-match-details">'+rows.map(x=>{
   const assist=x.assist?'<span class="sport-assist">🎯 '+esc(st("assist"))+': '+esc(x.assist)+'</span>':"";
   const who=x.player?'<strong>'+esc(x.player)+'</strong>':"";
   const minute=x.minute?'<span class="sport-event-minute">'+esc(x.minute)+'</span>':"";
   const raw=!x.player&&x.text?'<span>'+esc(x.text)+'</span>':"";
   return '<div class="sport-event-row"><span class="sport-event-icon">'+detailIcon(x.type)+'</span><div class="sport-event-main">'+who+raw+assist+'</div>'+minute+'</div>';
 }).join("")+'</div>';
}
function matchHtml(m){
 ...
 return `<article class="sport-match-vertical ${live?"is-live":""}" id="sport-match-${esc(key).replace(/[^a-zA-Z0-9_-]/g,"-")}">
   ...
   <button class="sport-details-btn" type="button" data-match-details="${esc(key)}">📋 ${esc(st("details"))}</button>
   ${detailHtml(key)}
 </article>`;
}
...
function mainButtons(){
 return `<div class="sport-main-actions">
   ...
   <button class="secondary sport-main-btn ${mode==="calendar"?"active":""}" data-sport-mode="calendar">🗓️ ${esc(st("calendar"))}</button>
...
function render(){
 ...
 if(mode==="days"){
   ...
   body=`<section class="card sport-calendar-card">
     <label class="sport-calendar-label" for="sportCalendarDate">${esc(st("chooseDate"))}</label>
     <input id="sportCalendarDate" class="sport-calendar-input" type="date" value="${esc(selectedCalendarDate)}">
     <div class="sport-section-title"><strong>${esc(prettyDateKey(selectedCalendarDate))}</strong></div>
     <div class="sport-day-content">${matchesSection("",matches)}</div>
   </section>`;
...
function bind(){
 ...
 root.querySelectorAll("[data-sport-mode]").forEach(b=>b.addEventListener("click",()=>{mode=b.dataset.sportMode;selectedLeague=null;if(mode==="days"||mode==="history"){selectedDay=0;loadMatches(true,false);}else if(mode==="calendar"){selectedCalendarDate=new Date().toISOString().slice(0,10);loadMatches(true,false);}else if(mode==="leagues"){loadLeagues();}else{selectedDay=0;loadMatches(true,false);}render();}));
 ...
 root.querySelectorAll("[data-match-details]").forEach(b=>b.addEventListener("click",()=>toggleMatchDetails(b.dataset.matchDetails)));
 document.getElementById("sportCalendarDate")?.addEventListener("change",e=>{selectedCalendarDate=e.target.value||new Date().toISOString().slice(0,10);loadMatches(true,false);});
...
async function toggleMatchDetails(key){
 if(expandedDetails.has(key)){expandedDetails.delete(key);render();return;}
 expandedDetails.add(key);render();
 if(matchDetails.has(key)||detailLoading.has(key))return;
 const m=matches.find(x=>eventKey(x)===key);if(!m)return;
 detailLoading.add(key);render();
 try{
   const source=m.source==="sportsdb"?"sportsdb":"espn";
   const eventId=String(m.event_id||String(m.id||"").replace(/^sdb-|^espnall-|^espn-/,""));
   const slug=String(m.league_slug||"")||(String(m.league_id||"").startsWith("espn:")?String(m.league_id).slice(5):"");
   const r=await fetch(API+"?action=details&source="+encodeURIComponent(source)+"&event="+encodeURIComponent(eventId)+"&slug="+encodeURIComponent(slug)+"&t="+Date.now(),{cache:"no-store"});
   const d=await r.json();matchDetails.set(key,Array.isArray(d?.events)?d.events:[]);
 }catch(e){console.warn("sport details",e);matchDetails.set(key,[]);}
 detailLoading.delete(key);render();
}
async function loadMatches(force=false,silent=false){
 if(loading)return;
 loading=true;if(!silent)render();
 try{
   const res=await fetch(API+"?date="+encodeURIComponent(requestDate())+"&t="+Date.now(),{cache:"no-store"});
...
function activate(){
 ...
 autoTimer=setInterval(()=>{if(document.getElementById("sportView")?.classList.contains("hidden"))return;if(mode==="today"||(mode==="days"&&selectedDay===0)||(mode==="calendar"&&selectedCalendarDate===new Date().toISOString().slice(0,10)))loadMatches(true,true);},3000);
}
