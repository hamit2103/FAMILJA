...
function incomingRequest(req){
 ...
 requestModal=document.createElement("div");requestModal.className="nearby-consent-overlay";
 if(req.call)startRingTone();
 const actions=req.call
   ? '<button id="nearbyConsentNo" class="secondary" type="button">'+esc(tx("decline"))+'</button><button id="nearbyConsentNow" class="primary" type="button">'+esc(tx("acceptCall"))+'</button>'
   : '<button id="nearbyConsentNo" class="secondary" type="button">'+esc(tx("decline"))+'</button><button id="nearbyConsentNow" class="secondary" type="button">'+esc(tx("allowNow"))+'</button><button id="nearbyConsentAlways" class="primary" type="button">'+esc(tx("allowAlways"))+'</button>';
 requestModal.innerHTML='<div class="nearby-consent-card"><h3>'+icons+'</h3><p><strong>'+esc(title)+'</strong><br>'+esc(partnerName||"")+'</p><div class="nearby-consent-actions">'+actions+'</div></div>';
 document.body.appendChild(requestModal);
