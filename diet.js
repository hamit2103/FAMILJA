...
function profile(){const p=read(DIET_PROFILE_KEY,{name:"",weight:0,target:0});return {...p,bmi:p.bmi??"",targetWeight:Number(p.targetWeight||0),targetDate:String(p.targetDate||""),startedAt:String(p.startedAt||""),goalDirection:String(p.goalDirection||"lose")};}
...
function render(){
 ...
 const p=profile(),items=todayLogs(),day=dayJournal(),used=items.reduce((s,x)=>s+Number(x.kcal||0),0),target=Number(p.target||0),remain=target?target-used:0;
 const pct=target?Math.min(100,Math.round(used/target*100)):0,started=p.startedAt||nowLocalInput();
 root.innerHTML=`
 ...
 <section class="card diet-profile diet-goal-card">
   <div class="diet-field-grid">
     ...
     <label>${esc(dt("kcalTarget"))}<input id="dietKcalTarget" type="number" min="0" max="10000" step="10" value="${target||""}"></label>
     <label>${esc(dt("bmi"))}<input id="dietBmi" type="number" min="0" max="100" step="0.1" value="${esc(p.bmi)}"></label>
 ...
 <section class="diet-summary">
   ...
   <div class="card diet-metric"><small>${esc(dt("bmi"))}</small><strong>${p.bmi!==""?esc(p.bmi):"—"}</strong><span>BMI</span></div>
 ...
 document.getElementById("dietSave")?.addEventListener("click",()=>{const saved={name:document.getElementById("dietName")?.value.trim()||"",weight:numValue("dietWeight"),target:numValue("dietKcalTarget"),bmi:document.getElementById("dietBmi")?.value||"",goalDirection:document.getElementById("dietGoalDirection")?.value||"lose",targetWeight:numValue("dietTargetWeight"),targetDate:document.getElementById("dietTargetDate")?.value||"",startedAt:document.getElementById("dietStartedAt")?.value||""};write(DIET_PROFILE_KEY,saved);const status=document.getElementById("dietStatus");if(status)status.textContent=dt("saved");setTimeout(render,250);});
