...
-- Multiple shared radio stations. Everyone can choose; only admin can manage the list.
create table if not exists public.radio_stations (
  id bigint generated by default as identity primary key,
  title text not null check (char_length(title) between 1 and 80),
  stream_url text not null check (char_length(stream_url) between 8 and 1000),
...
grant select, insert, update, delete on table public.radio_stations to authenticated;

do $$
declare
  seq_name text;
begin
  select pg_get_serial_sequence('public.radio_stations', 'id') into seq_name;
  if seq_name is not null then
    execute format('grant usage, select on sequence %s to authenticated', seq_name);
  end if;
end $$;

drop policy if exists "Family can read radio stations" on public.radio_stations;
create policy "Family can read radio stations"
on public.radio_stations
for select
to authenticated
using (true);

drop policy if exists "Admin can add radio stations" on public.radio_stations;
create policy "Admin can add radio stations"
on public.radio_stations
for insert
to authenticated
with check (auth.email() = 'admin@familja.local');

drop policy if exists "Admin can update radio stations" on public.radio_stations;
create policy "Admin can update radio stations"
on public.radio_stations
for update
to authenticated
using (auth.email() = 'admin@familja.local')
...
drop policy if exists "Admin can delete radio stations" on public.radio_stations;
create policy "Admin can delete radio stations"
on public.radio_stations
for delete
to authenticated
using (auth.email() = 'admin@familja.local');
...
-- DIAMOND 6.18 recipes and radio groups
alter table public.radio_stations
  add column if not exists station_group text not null default 'general';

alter table public.radio_stations
  drop constraint if exists radio_stations_station_group_check;
alter table public.radio_stations
  add constraint radio_stations_station_group_check
  check (station_group in ('general','kosovo-national','kosovo-local','turkish-nostalgia'));

create table if not exists public.recipes (
  id bigint generated by default as identity primary key,
  title text not null check (char_length(title) between 1 and 120),
  country_group text not null check (country_group in ('xk','tr')),
...
drop policy if exists "Family can read recipes" on public.recipes;
create policy "Family can read recipes" on public.recipes for select to authenticated using (true);
drop policy if exists "Admin can add recipes" on public.recipes;
create policy "Admin can add recipes" on public.recipes for insert to authenticated
with check ((select auth.email())='admin@familja.local');
drop policy if exists "Admin can update recipes" on public.recipes;
create policy "Admin can update recipes" on public.recipes for update to authenticated
using ((select auth.email())='admin@familja.local')
with check ((select auth.email())='admin@familja.local');
drop policy if exists "Admin can delete recipes" on public.recipes;
create policy "Admin can delete recipes" on public.recipes for delete to authenticated
using ((select auth.email())='admin@familja.local');
